...
class LeMigration(BaseModel):
    ...
    def update_notice(self, _migration_id, notice=None, pid=None, mode=None, status=None, finish=False,
        ...
        return getattr(self.get_abstract_migration(), 'update_notice')(_migration_id, notice, pid, mode, status, finish,
                                                                       clear_entity_warning)
    ...
    def get_migration_history(self, migration_id):
        if not migration_id:
            return True
        self._migration_id = migration_id
        query = "SELECT * FROM " + TABLE_MIGRATION_HISTORY + " WHERE migration_id = " + to_str(
            migration_id) + " AND type NOT LIKE '%demo%'"
    ...
    def set_flag_stop(self, migration_id, kill_all=False):
        if not migration_id:
            return response_success()
        self.delete_obj(TABLE_FLAG_STOP, {'migration_id': migration_id})
        return self.insert_obj(TABLE_FLAG_STOP,
                               {'migration_id': migration_id, 'flag': FLAG_STOP if not kill_all else FLAG_KILL_ALL})
    ...
    def create_demo_error(self, notice):
        ...
        if not check:
            ...
            for entity, process in self._notice['process'].items():
                if process['error']:
                    error_detail += "\n" + entity.capitalize() + ': ' + to_str(process['error']) + '/' + to_str(
                        process['imported'])
