...
class Mysql:
    ...
    def set_config(self, config):
        if config:
            ...
            config = configparser.ConfigParser()
            config.read(config_file)
            self.set_db_host(config[self.SECTION_CONFIG]['db_host']).set_username(
                config[self.SECTION_CONFIG]['db_username']).set_db_password(
                config[self.SECTION_CONFIG]['db_password']).set_db_prefix(config[self.SECTION_CONFIG]['db_prefix'])
