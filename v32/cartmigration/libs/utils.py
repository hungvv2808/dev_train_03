...
def start_subprocess(migration_id=None, buffer=None, wait=False):
    list_special = ['reset_migration', 'clone_migration', 'stop_auto_test', 'restart_migration',
                    'kill_end_loop_migration', 'kill_migration', 'delete_migration']
    action = buffer.get('action')
    if not migration_id and isinstance(buffer, dict) and get_value_by_key_in_dict(buffer, 'data', dict()).get(
            'migration_id'):
