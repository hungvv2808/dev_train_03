...
class BaseModel:
    ...
    def __init__(self):
        ...
        self._list_table_center = [TABLE_MIGRATION, TABLE_AUTO_TEST, TABLE_MIGRATION_TEST, TABLE_SETTING,
                                   TABLE_DEMO_ERROR]
    ...
    def get_db(self, config=None, test=False):
        if self._db:
            return self._db
        if (hasattr(self, '_mode') and getattr(self, '_mode') == 'test') or test or (
                hasattr(self, '_notice') and self._notice and self._notice['config'].get('test')):
            test = True
